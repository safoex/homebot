...
from telepot.aio.delegate import (
    per_chat_id, per_application, call, create_open, pave_event_space, include_callback_query_chat_id)
...
class UserStore(object):
    ...
    def __contains__(self, userid):
        c = self._conn.cursor()
        print(userid)
        r = c.execute('SELECT * FROM users WHERE userid=?',[userid]).fetchall()
        return True if len(r)>0 else False

    def add(self, userid, name):
        c = self._conn.cursor()
        r = c.execute('INSERT INTO users VALUES (?,?)',[userid, name])
        self._conn.commit()
    
    def edit_name(self, userid, newname):
        c = self._conn.cursor()
        r = c.execute('UPDATE users SET name=? WHERE userid=?',[newname, userid])
        self._conn.commit()

    def get_name(self, userid):
        c = self._conn.cursor()
        #TODO : try .. except with UnknownUserError
        r = c.execute('SELECT name FROM users WHERE userid=?',[userid]).fetchall()
        return r[0]
...
class NameSaver(telepot.aio.helper.ChatHandler):
    def __init__(self, seed_tuple, userstore, **kwargs):
        super(NameSaver, self).__init__(seed_tuple, **kwargs)
        self._userstore = userstore
        self._statemachine = dict()
    
    async def on_chat_message(self, msg):
        content_type, chat_type, chat_id = telepot.glance(msg)
         
        if content_type != 'text':
            await self.sender.sendMessage('Я уполномочен только узнать Ваше имя.')
            print('Irrelevant content ignored from %d\n' % chat_id)
            return
        
        if chat_id in self._statemachine:
            state = self._statemachine[chat_id]
            if state is 'newasked':
                self._userstore.add(chat_id, msg['text'])
                self._statemachine[chat_id] = 'known'
                await self.sender.sendMessage('Спасибо, %s! Такое счастье --  узнать Ваше имя!' % self._userstore.get_name(chat_id))
                return
            if state is 'known':
                await self.sender.sendMessage('Извините, %s, но я должен был всего лишь узнать Ваше имя. Больше я ничего не могу для Вас сделать! :(' % self._userstore.get_name(chat_id))
                return
        else:
            if chat_id in self._userstore:
                self._statemachine[chat_id] = 'known'
                await self.sender.sendMessage('Извините, %s, но я должен был всего лишь узнать Ваше имя. Больше я ничего не могу для Вас сделать! :(' % self._userstore.get_name(chat_id))
                return
            else:
                self._statemachine[chat_id] = 'newasked'
                await self.sender.sendMessage('Как Вас зовут?')
                return
